...
constexpr int MAX_LEN = 100; // 문자열 최대 길이 제한
...
string input2(string w, string wk)
{
    ...
    {
        twoBest[index2].push_back({w, point});
        std::sort(twoBest[index2].begin(), twoBest[index2].end());
    }
    ...
    if (twoBest[index2].size() == 10)
    {
        if (twoBest[index2].back().point < point)
        {
            twoBest[index2].pop_back();
            twoBest[index2].push_back({w, point});
            std::sort(twoBest[index2].begin(), twoBest[index2].end());
        }
    }
